...
function App() {
  ...
  const CustomAnalyticsTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const cbslVal = getCbslRate(dataPoint, activeCurrency);
      const cbslRateVal = compareMetric === 'buy' ? cbslVal.buy : cbslVal.sell;
        ...
        <div className="theme-panel p-4 rounded-xl border border-border bg-card text-card-foreground shadow-premium flex flex-col gap-1.5 min-w-[210px] text-xs">
          ...
          {cbslVal.buy > 0 && (
            <div className="flex justify-between items-center gap-4 border-t border-border mt-1 pt-1">
              ...
              <span className="font-semibold font-mono text-slate-500 dark:text-slate-400">{formatLKR(cbslVal.buy)}</span>
            </div>
          )}
          {cbslVal.sell > 0 && (
            <div className="flex justify-between items-center gap-4">
              ...
              <span className="font-semibold font-mono text-slate-500 dark:text-slate-400">{formatLKR(cbslVal.sell)}</span>
            </div>
          )}
          <div className="flex justify-between items-center gap-4 border-t border-border mt-1 pt-1">
  ...
  return (
    <div className="min-h-screen pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto select-none transition-colors duration-300">
      ...
      <header className="relative pt-6 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 z-20">
        <div>
          <div className="flex flex-wrap items-center gap-2.5 mb-1">
            <div className="flex items-center justify-center w-8 h-8 rounded-lg border border-border bg-card text-foreground shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5 self-stretch md:self-auto justify-between sm:justify-start">
          ...
          <div className="flex items-center gap-1.5 px-3 h-8 rounded-lg bg-card border border-border shadow-sm">
            <span className="text-muted-foreground font-semibold text-[10px] uppercase tracking-wider">Currency:</span>
            <select
            ...
            >
              ...
              <option value="SGD" className="bg-card text-foreground">SGD (S$)</option>
              <option value="AUD" className="bg-card text-foreground">AUD (A$)</option>
              <option value="CHF" className="bg-card text-foreground">CHF (CHF)</option>
              <option value="KWD" className="bg-card text-foreground">KWD (KD)</option>
              <option value="OMR" className="bg-card text-foreground">OMR (RO)</option>
              <option value="SAR" className="bg-card text-foreground">SAR (SR)</option>
              <option value="AED" className="bg-card text-foreground">AED (DH)</option>
              <option value="QAR" className="bg-card text-foreground">QAR (QR)</option>
              <option value="JOD" className="bg-card text-foreground">JOD (JD)</option>
              <option value="BHD" className="bg-card text-foreground">BHD (BD)</option>
              <option value="INR" className="bg-card text-foreground">INR (Rs)</option>
              <option value="CAD" className="bg-card text-foreground">CAD (C$)</option>
              <option value="NZD" className="bg-card text-foreground">NZD (NZ$)</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="flex items-center justify-center w-8 h-8 rounded-lg bg-card border border-border text-muted-foreground hover:text-foreground transition-all shadow-sm"
            ...
            <button 
              onClick={fetchData}
              disabled={loading}
              className="flex items-center justify-center w-8 h-8 rounded-lg bg-card border border-border text-muted-foreground hover:text-foreground transition-all shadow-sm disabled:opacity-50"
      ...
      <nav className="mb-6 border-b border-border flex items-center justify-start gap-1 pb-px overflow-x-auto no-scrollbar z-10">
        {[
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 border-b-2 text-xs font-semibold transition-all shrink-0 -mb-px ${
                isActive
      ...
      {successToast && (
        <div className="fixed bottom-6 right-6 z-50 theme-panel p-4 rounded-xl border border-buy/30 bg-card text-card-foreground shadow-premium max-w-md animate-bounce">
          <div className="flex items-start gap-3">
            <div className="p-1 rounded-md bg-buy/10 text-buy mt-0.5">
      ...
      {loading && data.length === 0 ? (
        ...
        <div className="theme-panel p-8 rounded-2xl border border-destructive/20 bg-card text-center max-w-xl mx-auto my-12">
          ...
          <button 
            onClick={fetchData}
            className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-semibold text-xs transition-all shadow-sm flex items-center gap-1.5 mx-auto hover:scale-95 active:scale-90"
          >
            ...
            <div className="animate-fadeIn">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 p-4 rounded-xl bg-card border border-border shadow-sm">
                <div className="flex items-center gap-3">
                  <span className="text-muted-foreground font-semibold text-xs uppercase tracking-wider">Active Bank Target</span>
                  <select
                    value={selectedBank}
                    onChange={(e) => setSelectedBank(e.target.value)}
                    className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-foreground focus:outline-none focus:ring-1 focus:ring-primary/20 transition-all cursor-pointer"
                  >
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-muted-foreground border-t sm:border-t-0 pt-2 sm:pt-0 border-border self-stretch sm:self-auto justify-between">
                  <span>Last Checked: <span className="text-foreground font-bold">{new Date(currentMetrics.timestamp).toLocaleDateString(undefined, {month: 'short', day: 'numeric', year: 'numeric'})}</span></span>
                  <span className="w-1.5 h-1.5 rounded-full bg-border"></span>
                  <span>Logs Size: <span className="text-foreground font-bold">{currentMetrics.totalEntries} Days</span></span>
                </div>
              </div>
                  ...
                  <div className="mt-2.5 border-t border-border pt-2 flex flex-col gap-1 text-[10px]">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-slate-500 dark:text-slate-400">
                        {currentMetrics.buyDiffVsGoogle.toFixed(2)} LKR
                      </span>
                    </div>
                    {currentMetrics.cbslRate.buy > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`font-semibold ${currentMetrics.buyDiffVsCbsl >= 0 ? 'text-buy' : 'text-rose-500 font-bold'}`}>
                          {currentMetrics.buyDiffVsCbsl >= 0 ? '+' : ''}{currentMetrics.buyDiffVsCbsl.toFixed(2)} LKR
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="theme-panel theme-panel-hover p-4 rounded-2xl bg-card text-card-foreground">
                  ...
                  <div className="mt-2.5 border-t border-border pt-2 flex flex-col gap-1 text-[10px]">
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground font-medium">vs Google Interbank:</span>
                      <span className="font-semibold text-slate-500 dark:text-slate-400">
                        +{currentMetrics.sellDiffVsGoogle.toFixed(2)} LKR
                      </span>
                    </div>
                    {currentMetrics.cbslRate.sell > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`font-semibold ${currentMetrics.sellDiffVsCbsl <= 0 ? 'text-buy' : 'text-rose-500 font-bold'}`}>
                          {currentMetrics.sellDiffVsCbsl > 0 ? '+' : ''}{currentMetrics.sellDiffVsCbsl.toFixed(2)} LKR
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="theme-panel p-5 rounded-2xl bg-card border border-border mb-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-border">
                  ...
                  {/* Range Selector */}
                  <div className="inline-flex p-1 rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 self-stretch sm:self-auto justify-between">
                    {[
                      { label: '7 Days', val: '7d' },
                  ...
                  {formattedSingleChartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <XAxis 
                          dataKey="timestamp" 
                          tickFormatter={(ts) => new Date(ts).toLocaleDateString(undefined, {
                            month: 'short',
                            day: 'numeric'
                          })}
                ...
                {historicalMetrics && (
                  <div className="theme-panel p-5 rounded-2xl bg-card text-card-foreground flex flex-col justify-between">
                    ...
                    <div className="mt-4 p-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] text-muted-foreground leading-relaxed flex gap-2">
                      ...
                      <span>
                        Margins dynamically parsed across database sequences. Averages indicate standard overhead offsets relative to interbank liquidity benchmarks.
                      </span>
                    </div>
                  </div>
                )}
                      ...
                      <div className="flex flex-wrap items-center gap-2 self-stretch sm:self-auto">
                        <div className="relative shrink-0">
                          ...
                          <input
                            ...
                            className="w-32 pl-8 pr-3 py-1 text-[11px] rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-foreground placeholder-muted-foreground focus:outline-none transition-all"
                          />
                        </div>
                        <button
                          onClick={exportToCSV}
                          className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold rounded-lg border border-border bg-card hover:bg-zinc-100 dark:hover:bg-zinc-800 text-foreground transition-all shadow-sm"
                      ...
                      <table className="w-full text-left text-[11px] border-collapse">
                        <thead>
                          <tr className="border-b border-border text-muted-foreground font-bold uppercase tracking-wider">
                            <th className="py-2 px-2">Date Logged</th>
                            <th className="py-2 px-2 text-right">Bank Buy</th>
                            <th className="py-2 px-2 text-right">Bank Sell</th>
                            <th className="py-2 px-2 text-right">Spread</th>
                            <th className="py-2 px-2 text-right">Google Interbank</th>
                          </tr>
                        </thead>
                            ...
                            paginatedTableData.map((item, idx) => {
                              ...
                              return (
                                <tr key={idx} className="hover:bg-zinc-150/40 dark:hover:bg-zinc-900/60 text-muted-foreground hover:text-foreground transition-colors">
                                  <td className="py-2.5 px-2 font-medium flex items-center gap-1.5 text-foreground">
                                  ...
                                  <td className="py-2.5 px-2 text-right font-mono text-buy font-semibold">{formatLKR(buy)}</td>
                                  <td className="py-2.5 px-2 text-right font-mono text-sell font-semibold">{formatLKR(sell)}</td>
                                  <td className="py-2.5 px-2 text-right font-mono text-spread font-semibold">{formatLKR(spread)}</td>
                                  <td className="py-2.5 px-2 text-right font-mono text-google font-semibold">{formatLKR(googleRate)}</td>
                                </tr>
                              );
                            })
                          ) : (
            ...
            <div className="animate-fadeIn">
              ...
              <div className="theme-panel p-5 rounded-2xl bg-card text-card-foreground border border-border mb-6">
                    ...
                    <div className="inline-flex p-1 rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
                      <button
                        ...
                        className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${
                          compareMetric === 'buy'
                            ? 'bg-card text-buy border border-border shadow-sm'
                            : 'text-muted-foreground hover:text-foreground'
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${
                          compareMetric === 'sell'
                            ? 'bg-card text-sell border border-border shadow-sm'
                            : 'text-muted-foreground hover:text-foreground'
                        }`}
                      >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mb-6 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground font-semibold text-[10px] uppercase tracking-wider">Compare Targets:</span>
                      ...
                      {Object.keys(bankNames).map(bk => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[11px] font-semibold transition-all active:scale-95 shadow-sm ${
                              isChecked
                          ...
                          >
                            ...
                            <span>{bk === 'cbsl' ? 'CBSL' : bk === 'combank' ? 'ComBank' : bk === 'peoples' ? "People's" : bk === 'seylan' ? 'Seylan' : bk === 'union' ? 'Union' : bk === 'amana' ? 'Amana' : bk === 'dfcc' ? 'DFCC' : bk === 'nsb' ? 'NSB' : bk === 'pabc' ? 'PABC' : bk === 'ndb' ? 'NDB' : bankNames[bk].split(' ')[0]}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="text-[10px] text-muted-foreground font-medium hidden sm:block">
                    Toggle pills to filter comparative chart lines and side-by-side matrices.
                  </div>
                </div>
                ...
                <div className="w-full h-[320px] sm:h-[350px]">
                  {formattedCompareChartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={formattedCompareChartData}
                        margin={{ top: 10, right: 10, left: -20, bottom: 0 }}
                      >
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border) / 0.4)" vertical={false} />
                        <XAxis 
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-border text-muted-foreground font-bold uppercase tracking-wider text-[10px]">
                        <th className="py-2.5 px-3">Sri Lankan Bank</th>
                        <th className="py-2.5 px-3 text-center">7-Day Trend</th>
                        <th className="py-2.5 px-3 text-right">Buying Rate</th>
                        <th className="py-2.5 px-3 text-right">Buy Difference vs Google</th>
                        <th className="py-2.5 px-3 text-right">Selling Rate</th>
                        <th className="py-2.5 px-3 text-right">Sell Difference vs Google</th>
                        <th className="py-2.5 px-3 text-right">Spread (LKR)</th>
                    ...
                    <tbody className="divide-y divide-border">
                      {comparisonMatrix.map(row => (
                        <tr key={row.key} className="hover:bg-zinc-150/40 dark:hover:bg-zinc-900/60 text-muted-foreground hover:text-foreground transition-all">
                          ...
                          {/* Buy Rate */}
                          <td className={`py-3 px-3 text-right font-mono font-bold ${row.isBestBuy ? 'text-buy' : 'text-foreground'}`}>
                            {formatLKR(row.buy)}
                          </td>
                          
                          {/* Buy Margin vs Google */}
                          <td className="py-3 px-3 text-right font-mono">
                            <span className="inline-flex items-center gap-0.5 bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 rounded-md border border-zinc-200 dark:border-zinc-800 text-[10px]">
                              {row.buyDiffVsGoogle.toFixed(2)} LKR
                            </span>
                          </td>
                          
                          {/* Sell Rate */}
                          <td className={`py-3 px-3 text-right font-mono font-bold ${row.isBestSell ? 'text-sell' : 'text-foreground'}`}>
                            {formatLKR(row.sell)}
                          </td>
                          ...
                          {/* Spread LKR */}
                          <td className={`py-3 px-3 text-right font-mono font-semibold ${row.isBestSpread ? 'text-spread' : ''}`}>
                            {formatLKR(row.spread)}
                          </td>
                ...
                <div className="mt-4 p-4 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] text-muted-foreground leading-relaxed flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-4 space-y-1">
                      <li>
                        For **Buying Rates**, higher is better. A higher buy rate means the bank pays you more LKR when purchasing your USD. The bank with the highest buying price receives the <strong className="text-buy">★ Best Buy</strong> badge.
                      </li>
                      <li>
                        For **Selling Rates**, lower is better. A lower sell rate means you pay less LKR to buy USD from the bank. The bank with the lowest selling price receives the <strong className="text-sell">★ Best Sell</strong> badge.
                      </li>
                      <li>
                        A tighter spread represents more competitive internal bank currency operations.
                      </li>
                    </ul>
                  </div>
                </div>
                ...
                <div className="theme-panel p-5 rounded-2xl bg-card text-card-foreground border border-border md:col-span-1 flex flex-col justify-between">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Amount to Convert</label>
                      <div className="relative">
                        <input
                          type="number"
                          value={calculatorAmount}
                          onChange={(e) => setCalculatorAmount(Math.max(1, parseFloat(e.target.value) || 0))}
                          className="w-full px-3 py-2 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-bold focus:outline-none focus:ring-1 focus:ring-primary/20 text-foreground"
                          min="1"
                        />
                    ...
                    <div className="space-y-1">
                      <span className="text-[9px] font-semibold text-muted-foreground">QUICK PRESETS</span>
                    ...
                    {/* Direction Switcher */}
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Conversion Mode</label>
                  ...
                  <div className="mt-4 p-3 rounded-xl bg-zinc-150/40 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] text-muted-foreground leading-relaxed flex gap-2">
                    <Info className="w-3.5 h-3.5 text-muted-foreground shrink-0 mt-0.5" />
                    <span>Calculations automatically incorporate live, direct exchange rate quotes for {activeCurrency} refreshed from current flat-file database registries.</span>
                  </div>
                </div>
                ...
                <div className="theme-panel p-6 rounded-2xl md:col-span-2 border border-border flex flex-col justify-between relative overflow-hidden bg-gradient-to-br from-zinc-900 via-zinc-950 to-zinc-900 text-white dark:border-zinc-800/80 shadow-premium min-h-[300px]">
                  ...
                  <div className="z-10">
                    ...
                    {calculatorResults.length > 0 ? (
                      <div className="space-y-4">
                        <div>
                          ...
                          <h2 className="text-3xl font-black text-white tracking-tight mt-1 flex items-baseline gap-2">
                            ...
                            <span className="text-xs text-emerald-400 font-bold bg-emerald-500/15 border border-emerald-500/25 px-2 py-0.5 rounded-md uppercase tracking-wider">Optimal Option</span>
                          </h2>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                            ...
                            <span className="text-2xl font-black text-white font-mono mt-1 block">
                              {formatLKR(calculatorResults[0].totalLKR)}
                            </span>
                            <span className="text-[10px] text-zinc-400 font-semibold mt-0.5 block">
                              At rate of Rs. {calculatorResults[0].rate.toFixed(2)} per unit
                            </span>
                          </div>
                          ...
                          <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col justify-between">
                            <div>
                              ...
                              <span className="text-xl font-extrabold text-emerald-400 font-mono mt-1 block">
                                {formatLKR(calculatorResults[0].maxDifference)}
                              </span>
                            </div>
                            <span className="text-[9px] text-zinc-400 font-semibold block mt-1 leading-normal">
                              Extra money saved compared to the worst choice: **{calculatorResults[calculatorResults.length - 1].name}** ({formatLKR(calculatorResults[calculatorResults.length - 1].totalLKR)}).
                            </span>
                          </div>
                        </div>
                        ...
                        {marketSignal && (
                          <div className="mt-4 p-3.5 rounded-xl bg-white/5 border border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 backdrop-blur-sm shadow-inner">
                            ...
                            <div className="shrink-0 flex flex-col items-end justify-center text-right font-mono bg-black/30 px-3 py-2 rounded-lg border border-white/5 min-w-[125px]">
                      ...
                      <div className="py-8 text-center text-zinc-400 text-xs">
                        No conversion parameters resolved. Please enter a valid currency quantity.
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-white/10 z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-[10px] text-zinc-400 font-medium">
                    ...
                    <span className="text-zinc-500">
                      Calculations are client-side simulations. Indicative rates only.
                    </span>
                  </div>
              ...
              <div className="theme-panel p-5 rounded-2xl bg-card text-card-foreground border border-border">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 pb-2 border-b border-border">
                  <div>
                    ...
                    <p className="text-muted-foreground text-[10px] mt-0.5">Institutions ranked from best value payoff (Rank #1) to worst, incorporating absolute deflection margins.</p>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y divide-border">
                      {calculatorResults.map(row => (
                        <tr key={row.key} className="hover:bg-zinc-150/40 dark:hover:bg-zinc-900/60 text-muted-foreground hover:text-foreground transition-all">
                          ...
                          <td className="py-3 px-3 text-right font-mono">
                            {row.isBest ? (
                              <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/15 border border-emerald-500/20 px-2 py-0.5 rounded-md">Optimal (Rs. 0.00)</span>
                            ) : (
                ...
                <div className="mt-4 p-4 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-[10px] text-muted-foreground leading-relaxed flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-4 space-y-1">
                      <li>
                        When **selling foreign currency** (e.g. converting USD to LKR), the bank with the highest buy rate is optimal, as you receive more LKR in return.
                      </li>
                      <li>
                        When **buying foreign currency** (e.g. converting LKR to USD), the bank with the lowest sell rate is optimal, as you pay less LKR in return.
                      </li>
                      <li>
                        Relative value efficiency indicates the proportion of optimal returns matched by that specific institution. Deficits show exact absolute financial differences compared to the rank #1 optimal selection.
                      </li>
                    </ul>
                  </div>
                </div>
      ...
      <footer className="text-center text-muted-foreground text-[10px] mt-12 pt-5 border-t border-border leading-relaxed">
        ...
        <p className="mt-0.5 text-muted-foreground/60">
          Daily bank rates are indicative and subject to change. Developed fully statically via Jamstack principles.
        </p>
      </footer>
    </div>
  );
